...
BEGIN TRY
    ...
    IF OBJECT_ID(N'dbo.TaskDiscussion', N'U') IS NOT NULL AND NOT EXISTS (SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.TaskDiscussion') AND name=N'IX_TaskDiscussion_Task_Subtask') EXEC(N'CREATE INDEX IX_TaskDiscussion_Task_Subtask ON dbo.TaskDiscussion(Task_id,Subtask_id);');
    IF OBJECT_ID(N'dbo.TaskDiscussion', N'U') IS NOT NULL AND OBJECT_ID(N'dbo.FK_TaskDiscussion_Task_Subtask', N'F') IS NULL EXEC(N'ALTER TABLE dbo.TaskDiscussion WITH CHECK ADD CONSTRAINT FK_TaskDiscussion_Task_Subtask FOREIGN KEY (Task_id,Subtask_id) REFERENCES dbo.Subtask(Task_id,Subtask_id);');
